...
LaserMappingNode::LaserMappingNode(const rclcpp::NodeOptions &options) : Node("laser_mapping", options)
{
    ...
    angle_between_gimbal_and_chassis=0;
    ...
    sub_angle_error_ = this->create_subscription<rm_interfaces::msg::AngleError>("/robot/angleerror", 10,
       std::bind(&LaserMappingNode::angle_error_callback, this, std::placeholders::_1));
    ...
    base_link_tf_broadcaster = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
    double_yaw_tf_broadcaster = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
    ...
    angle_timestamp = this->get_clock()->now();
...
void LaserMappingNode::readParameters()
{
    ...
    RCLCPP_INFO(this->get_logger(), "-------------is_we_are_blue: %d----------------", is_we_are_blue_);
}
 ...
 * @param msg 角度误差消息
 */
void LaserMappingNode::angle_error_callback(const rm_interfaces::msg::AngleError::ConstPtr &msg)
{
    angle_between_gimbal_and_chassis = msg->angle_error;
    angle_timestamp = this->get_clock()->now();
    angle_between_small_yaw_and_big_yaw = msg->gimbal_error;
}
...
void LaserMappingNode::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMapped, std::unique_ptr<tf2_ros::TransformBroadcaster> & tf_br)
{
    ...
    // RCLCPP_INFO(this->get_logger(), "odometry transform: x: %lf, y: %lf, z: %lf, yaw: %lf",
    //             transformStamped.transform.translation.x, transformStamped.transform.translation.y,
    //             transformStamped.transform.translation.z, yaw);
    //     RCLCPP_INFO(this->get_logger(), "transformStamped.transform.rotationor: %f, %f, %f, %f", transformStamped.transform.rotation.x, transformStamped.transform.rotation.y, transformStamped.transform.rotation.z, transformStamped.transform.rotation.w);
    yaw += angle_between_gimbal_and_chassis;

    // 生成基于yaw角度的旋转四元数（围绕Z轴）
    tf2::Quaternion new_quat;
    new_quat.setRPY(roll, pitch, yaw); // 只有绕Z轴旋转，pitch和roll都为0

    // 将新的四元数赋值给transformStamped
    transformStamped.transform.rotation.x = new_quat.x();
    transformStamped.transform.rotation.y = new_quat.y();
    transformStamped.transform.rotation.z = new_quat.z();
    transformStamped.transform.rotation.w = new_quat.w();
    // RCLCPP_INFO(this->get_logger(), "transformStamped.transform.rotation: %f, %f, %f, %f", transformStamped.transform.rotation.x, transformStamped.transform.rotation.y, transformStamped.transform.rotation.z, transformStamped.transform.rotation.w);
    transformStamped.header.stamp = angle_timestamp;
    transformStamped.header.frame_id = map_frame;
    transformStamped.child_frame_id = "base_link"; //map->base_link
    base_link_tf_broadcaster->sendTransform(transformStamped);

    // /* 大yaw到小yaw的坐标系变换（自瞄专用） */
    geometry_msgs::msg::TransformStamped gimbal_transform;
    gimbal_transform.header.stamp = rclcpp::Time(odomAftMapped.header.stamp);
    gimbal_transform.header.frame_id = odom_frame;//map_frame;
    gimbal_transform.child_frame_id = "odom_yaw"; //小yaw，自瞄的odom
    gimbal_transform.transform.translation.x = 0.0; //odomAftMapped.pose.pose.position.x + 0.0126;
    gimbal_transform.transform.translation.y = 0.0; //odomAftMapped.pose.pose.position.y;
    gimbal_transform.transform.translation.z = 0.0;

    double gimbal_roll, gimbal_pitch, gimbal_yaw;
    gimbal_roll = 0;
    gimbal_pitch = 0;
    gimbal_yaw = -angle_between_small_yaw_and_big_yaw / 180 * M_PI;
    // RCLCPP_INFO(this->get_logger(), "gimbal_yaw: %f", gimbal_yaw);
    tf2::Quaternion gimbal_quat;
    gimbal_quat.setRPY(gimbal_roll, gimbal_pitch, gimbal_yaw);

    gimbal_transform.transform.rotation.x = gimbal_quat.x();
    gimbal_transform.transform.rotation.y = gimbal_quat.y();
    gimbal_transform.transform.rotation.z = gimbal_quat.z();
    gimbal_transform.transform.rotation.w = gimbal_quat.w();

    // gimbal_transform.transform.rotation.x = 0.0;
    // gimbal_transform.transform.rotation.y = 0.0;
    // gimbal_transform.transform.rotation.z = 0.707107;
    // gimbal_transform.transform.rotation.w = 0.707107;
    double_yaw_tf_broadcaster->sendTransform(gimbal_transform);
    // RCLCPP_INFO(this->get_logger(), "gimbal_transform.transform.rotation: %f, %f, %f, %f",
    //             gimbal_transform.transform.rotation.x, gimbal_transform.transform.rotation.y,
    //             gimbal_transform.transform.rotation.z, gimbal_transform.transform.rotation.w);
}
